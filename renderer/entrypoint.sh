...
export PGHOST=$MAPNIK_POSTGRES_DB_HOST
echo " ------------------------------------------------------------------------------------------- "
echo " VERIFICATION RENDERER ENNTRYPOINT : [MAPNIK_POSTGRES_USER=$MAPNIK_POSTGRES_USER] "
echo " VERIFICATION RENDERER ENNTRYPOINT : [MAPNIK_POSTGRES_DB=$MAPNIK_POSTGRES_DB] "
echo " VERIFICATION RENDERER ENNTRYPOINT : [MAPNIK_POSTGRES_DB_HOST=$MAPNIK_POSTGRES_DB_HOST] "
...
# but instead of this, I'd rather use a plain docker HEALTHCHECKC, with "depends_on" relationship btw containers
# psql -d gis -U $MAPNIK_POSTGRES_USER -h $MAPNIK_POSTGRES_DB_HOST -p 5432 -c "SELECT 1;"
# Reminds me of JDBC old days ... 

# And yo ugly, you just go away
